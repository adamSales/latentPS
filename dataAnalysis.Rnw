...
<<runmainModel,dependson='prelimMain'>>=
...
cpdf <- data.frame(Value=coefs[3:7,1],Coefficient=c('Black/Multiracial','Hispanic/Native American','Male','Special Ed.','Gifted'),HighInner=coefs[3:7,'75%'],LowInner=coefs[3:7,'25%'],HighOuter=coefs[3:7,'97.5%'],LowOuter=coefs[3:7,'2.5%'],Model='model')

coefplot.data.frame(cpdf,title=NULL,xlab=expression(hat(beta)[std]),ylab=NULL)+theme(text=element_text(size=20))
ggsave('figure/usageCoef.pdf',width=3,height=3)

pdf('figure/pretestEta.pdf')
omar <- par()$mar
par(mar=omar+c(0,1,-1,0))
plot(dat$xirt,colMeans(draws$studEff)/sdEta,col=ifelse(dat$treatment==1,'blue','red'),
...
stEf <- draws$studEff[samp,]
studEff95 <- quantile(stEf,c(0.025,0.975))
stEf[stEf<studEff95[1] | stEf>studEff95[2]] <- NA
trtEff <- sweep(sweep(stEf,1,draws$b1[samp],'*'),1,draws$b0[samp],'+')
