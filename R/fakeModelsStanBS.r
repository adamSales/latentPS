...
save(list=ls(),file='output/noEffectBS.RData'); rm(noEff); gc()
...
save(constEff,sdatF,file='output/constEffBS.RData')
...
save(linEff,sdatFlin,file='output/linEffBS.RData');rm(linEff);gc();
...
save(quadEff,sdatFquad,file='output/quadEffBS.RData'); rm(quadEff); gc();
