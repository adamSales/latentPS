...
noEff <- stan('R/psmodBC.stan',data=sdatF,iter=3000,chains=6)


save(list=ls(),file='output/noEffectBSBC.RData'); rm(noEff); gc()
...
constEff <- stan('R/psmodBC.stan',data=sdatFConst,iter=3000,chains=6)


save(constEff,sdatF,file='output/constEffBSBC.RData');rm(constEff);gc()
...
linEff <- stan('R/psmodBC.stan',data=sdatFlin,iter=3000,chains=6)


save(linEff,sdatFlin,file='output/linEffBSBC.RData');rm(linEff);gc();
...
quadEff <- stan('R/psmodBC.stan',data=sdatFquad,iter=3000,chains=6)





save(quadEff,sdatFquad,file='output/quadEffBSBC.RData'); rm(quadEff); gc();
